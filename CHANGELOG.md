...
**An entry that moves something on an installed machine names its guide**, `upgrades/12.md`. The entry stays short, because a user reads it. The guide holds every path that moves and how, because `/flow:migrate` reads that. `upgrades/README.md` holds the shape.
