...
Flow's tests write into `tmp/tests/`. `util`'s write into `lab/util/tmp/tests/`. Both are gitignored, and neither suite writes anywhere else.
...
They are set whether the test needs them or not. Left unset, a test that dispatches from inside this repository picks up a real project source, and a test that runs `util install` puts symlinks on the `PATH` of whoever ran the suite.
