...
- **`settings.json`**: the settings both your machines share. `reminder` is whether the reminder prints beside every message, `sessionCheck` whether a session opens with a line about what needs attention, and `setupReminder` whether a git repository without Flow gets a line suggesting it: one key per line Flow prints by itself. `sources` lists the skill repositories, and `skills` the skills switched on or off for every machine. `skillsAutoUpdate` is whether each skill repository pulls itself when a session opens.
