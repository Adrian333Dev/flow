...
A phase skill is one you type to work a ticket through one phase: `/groundwork` settles the open questions, `/execute` plans and builds, `/debug` finds a cause and fixes it, `/prototype` answers one question with throwaway code. There are 4 and no more. This page says what each one accepts after its name and what that loads. [Opening a session](start.md) says which ticket goes to which skill.
...
- **Nothing**: the phase opens on whatever is already in the conversation. This is how `/start` invokes a phase, since it has already loaded the ticket.
...
The first line of every phase skill runs `flow get <id> --files`, the command that prints one ticket and every file its `open` block names. What arrives, for `/execute t002`:
...
```text
/groundwork t005 start from the migration cost, the rest can wait
/groundwork we need to decide where uploads are stored
```
...
Each skill is a long file, and Claude Code adds it to the conversation whole. Running `/execute` bare and then `/execute t002` in the same session adds the text twice, because the second run renders differently, with the ticket in it. Typing the id the first time avoids the second copy. `/start t002` has the same effect, since the skill it invokes runs bare.
...
- **`/groundwork`** writes `groundwork/map.md`: every question, settled or open, and the reasoning under each. At the end it cuts tickets for what was decided and hands a feature to `/execute`.
- **`/execute`** writes `plan.md`, waits for your yes on the steps, builds them, and hands the work back at `review`.
- **`/debug`** writes `reports/<failure>.md`: the failing check, the hypotheses, the one that held, and the fix.
- **`/prototype`** writes `reports/<question>.md`: what was built, what it showed, and the answer. The code is deleted.
