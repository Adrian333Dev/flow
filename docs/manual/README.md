...
- Use Flow, one page per part of a working day:
  - [Opening a session](use/start.md): `/flow:start` with nothing, with a ticket id, with a path, and what each one loads
  ...
  - [Stopping and picking up](use/resume.md): `/flow:handoff`, `/clear`, then the skill with the id
...
Then the 4 Use Flow pages, in the order listed: they follow one ticket from `/flow:start` to the handoff.
