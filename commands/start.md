...
**A ticket is above.** Its `type:` line picks the skill, and `feature` and `chore` read the `status:` line too. Read nothing else first, then invoke the skill — it loads here, in this session.
