...
- A failure with an artifact — the user reacts to something you produced, or a loaded rule didn't fire → a study case. Keep the offending output verbatim first, analyse after: `~/.claude/flow/references/study-cases.md`
...
- `~/.claude/flow/references/workflow.md` — how Flow's pieces fit together. Only when that is genuinely unclear
- `~/.claude/flow/references/writing.md` — the house style: section shapes, sentence rules, what may never be cut
- `~/.claude/flow/references/cli-design.md` — the rules `flow`'s own surface follows. Only when adding a command, an action or a flag
- `~/.claude/flow/references/study-cases.md` — how to write one
