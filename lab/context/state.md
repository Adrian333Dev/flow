...
- **`tmp/try/`**: where `bash lab/scripts/try.sh` builds a throwaway install, under `tmp/try/root/`, which stands in for the home folder. It copies the credentials, the account and the theme from `~/.claude/`, so the session starts signed in. The install runs with `--no-clone`, so the scratch machine has no util, toolbox or skill repository clone. `--codex` starts Codex against the same install instead, with no rule file since 2026-09-23, and a copy of `~/.codex/auth.json`, and refuses when that login is less than a day from renewing, since a scratch renewal would sign the real Codex out. Set 2026-09-18. The scratch project is built from a seed under `lab/scripts/seeds/`, picked with `--seed <name>`: `app` (default, a small expense tracker with 9 tickets that fit it), `guards`, `resume` and `empty`. Set 2026-09-16.
