...
still open. The user said explicitly that no design here has their yes yet. `backlog.md` → `## V1`
carries the item. The shared corpus of domain skills, its private folder and the fold started here and
moved to `skills.md` on 2026-09-12, where every decision stood unopposed. The history that produced the
skill is under `## Where the idea came from` at the bottom, merged in from the deleted `threads.md` on
2026-09-16.
...
- **`flow install` does nothing on a machine that already has `~/.claude/CLAUDE.md`.**
  ...
  user arrives in. `## Parts the skill carries` below lists it.
...
## Parts the skill carries

Moved here from `backlog.md` on 2026-09-15, where each was a line of its own. Each is a job the skill has to do. None is designed.

- **A machine that already has `~/.claude/CLAUDE.md` gets none of Flow's rules.** The first fault above. Every real machine arrives this way, and the skill is what replaces that file.
- **The 2 personalised files are diffed on re-install.** `~/.claude/CLAUDE.md` and `~/.claude/settings.json` are copied, then personalised, so a new Flow version never reaches them. Every other path is a symlink and updates with `git pull`. A diff misses one case, raised by the user 2026-09-07: Flow v1 ships a rule, the user overrides it in their copy, Flow v2 deletes the rule, and the override survives pointing at nothing. P1 and P2 below answer it.
- **`util` is checked before anything else.** `home/CLAUDE.md` sends every look at structure through `util fs tree`, and `open.js` runs `util fs merge` off `PATH`. Depending on it is acceptable only while `util` is public. If it ever goes private, `ptree` and `fmerge` come back into Flow.
- **`docs/` is read before anything is written into it.** A project that already has `docs/spec/` or `docs/research/` gets merged, never overwritten. The skill creates `docs/spec/`, `docs/context/`, `docs/research/` and `docs/intake/`, and nothing else under `docs/`.
- **A repository the skill creates starts with `git init -b main`.** No shipped script runs `git init` today. `util` started on `master` from a bare `git init` while every other repository the user has is on `main`, and was renamed by hand 2026-08-31.
- **Built-in `/init` is tried first, with `CLAUDE_CODE_NEW_INIT=1`, on a real repo.** It already surveys a codebase, asks about the gaps and shows a proposal to review, so the skill may reuse it rather than rebuild it.
...
2. **Do `## The user` and `## Preferences` merge into one section, or stay split with a stated line
   between them?** This is item 3 in `backlog.md` → `## V1`, and the interview design depends on it.
3. **The skill's name and group.** `/manage-flow` under `dev/` was suggested and is the weakest
   recommendation on this page.

Answered elsewhere, in `skills.md`: how a domain skill installs, which is into one project
and never through `flow install`, plus `/distill` and every name.

## Where the idea came from

Raised by the user 2026-08-15, as one skill instead of two.

The plan had been 2 skills: `setup-flow-globals` to install Flow on a machine that has none, and
`migrate-to-flow` to convert a project that already has its own workflow. Neither was ever built.

**The user's reason for one skill**: the starting states are open-ended. A bare machine, a machine with
Flow and a fresh project, a project with its own `CLAUDE.md` and docs, a project half-converted. There is
no clean line to cut 2 skills along, and writing one skill per scenario means writing a skill per scenario
forever.

**Decided 2026-09-01: the skill manages Flow's whole life, and installing is one job inside that.** The
user's correction on seeing it scoped as an installer. It owns every starting state, then everything after
one: updating a machine, re-installing over the 2 personalised files, converting a project that already has
its own workflow, and every migration that follows. **It is far larger than the item written down for it**,
which is why it stays last.

**3 questions from that conversation are still unanswered**, and none of them is in `## Open questions`
above:

- **What the single skill branches on.** Whatever it detects first: global rules present or absent, project
  `CLAUDE.md` present or absent, foreign docs present or absent. Those 3 flags are 8 states, and most
  collapse.
- **Whether install and convert really share a spine**, or only share a trigger. Installing writes to
  `~/.claude/`; converting writes inside a repository. One skill doing both crosses the boundary every
  other Flow skill respects.
- **The name.** `setup-flow-globals` and `migrate-to-flow` are both verb-first and both describe half the
  job.
