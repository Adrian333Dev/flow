...
- **Only files git would keep are read.** Delapse's `temp/repos/` holds about 50 rule files of other projects.
- **The step file gives the harvest goals and tips, never a procedure.** Ruled by the user 2026-09-25, replacing 3 stages: a survey writing `inventory.md`, subagents reading it in batches of about 150 KB, and every source typed by content before it was read. The session starts with `util fs tree`, whose line counts show a large file before it is opened. It reads every rule file, every doc written for this project, the project's memory and its settings. It uses subagents where they help, with no rules on how. A stopped run reads again, and the findings already on disk stay.
- **The code is explored in its own right, and wins over every doc.** In many projects it is the only source, and even Delapse's docs leave out the stack and how the pieces fit. The session skips what doesn't matter, and never reads every file. A doc the code contradicts becomes a ticket.
- **Documentation copied in from elsewhere is left unread.** Delapse's largest doc is 860 KB of Inngest's own documentation.
- **Every finding lands in one place**: the project's `AGENTS.md`, `docs/context/`, a ticket, `.flow/inbox.md`, `.flow/findings/`, the machine's 2 sections, left in place, taken over as a skill source, or dropped with its Flow rule named.
...
- **A `CLAUDE.md` in a subfolder needs nothing of its own.** The harvest reads every rule file git keeps, so a committed `packages/api/CLAUDE.md` is sorted like the root one. A repository cloned in later is usually gitignored, and is handled when it causes trouble.
