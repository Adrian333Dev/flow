...
| **refs** | mine the repos cloned at `repos/` for ideas | pulled from as needed, not a session of its own |
...
**Every citation below to a numbered `remaining.md` item — `2c`, `2e`, `2f`, `2g` — or to a section of it
resolves through git.** That file was cut to its 2 locked sections on 2026-08-28, and the cited material
went with the rest. The argument each citation supports is stated here in full, so nothing needs fetching.
...
- `remaining.md` 2f: *"Haiku delegation and the debug-agent handoff survive as-is; `haiku-worker.md` path
  refs stay valid."*
- `home/CLAUDE.md` `## Workflow`: *"todo → in-progress → review → done, **Haiku subagents by default**."*
...
`home/CLAUDE.md` line 50 and `remaining.md` 2f both need correcting, and `execute/haiku-worker.md` is a
delete.**
...
- [ ] Output contract per agent — what it returns, in what shape. Model:
      `repos/caveman/agents/cavecrew-*.md`, which fix the format exactly.
...
- `home/CLAUDE.md`'s hard rule *"No cause without evidence. Hypothesis: X. To verify: Y."* is the
  debugging-side ancestor of the same instinct.
- `repos/agent-skills/skills/doubt-driven-development/` — a third-party skill squarely in this space,
  unread as of writing. Check it before designing from scratch.

### Settled 2026-08-13 — all four

- **Name.** `## Judgment`, a section in `home/CLAUDE.md`.
...
## refs — mine the reference repos at `repos/`

**Cloned by the user 2026-08-08, for inspiration.** Not vendored, not a dependency, not part of the product.
`lab/` is deleted when the build finishes and these go with it. Pulled from as the other threads need them,
...
**Reading any file inside `repos/<repo>/` auto-loads that repo's `CLAUDE.md` into context.** Confirmed by
...
`lab/research/claude-code-docs/` — `hooks.md`, `sub-agents.md` and `agent-view.md`.
...
**The whole thread lives in `lab/context/design-ascii-engine.md`** — what an engine is, the same diagram typed
...
On Phase 4: too detailed given `references/review-code.md` already exists.
...
read against the plan, then read again against the code through `references/review-code.md`, then the user approves
