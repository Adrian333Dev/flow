...
- **A skill given arguments re-appends its whole body.** An argument makes the render differ per invocation, so Claude Code appends the file again instead of skipping it. That is why `/execute t047` is unbuildable: `/groundwork` is 284 lines, `/execute` 191, `/handoff` 153. `backlog.md` → `## V1`, `skills.md` → `### Arguments`
