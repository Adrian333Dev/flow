...
Measured by the user on 2026-08-19, by eye, from `lab/research/ascii-glyph-probe.md`:
...
It now lives at `lab/research/ascii-glyph-probe.md`. The script above is still unbuilt.
...
- **SVG through the excalidraw skills: ~10 minutes and ~80k tokens per diagram**, measured, recorded in
  `lab/excalidraw/README.md`. That is what ASCII is competing against, and why it wins.
...
Shipped, in `skills/visualize/references/`:
...
- **`youtube-page.md`** — 150 × 58. UI-rich mockup: toolbar, 92-wide player at 16:9, description box with
  chapters, comments, a 9-card sidebar. The proportion and density case, pointed at from `references/draw-mockups.md`.
...
- **Stays in `SKILL.md`** — layered stack, pipeline, flow with return paths, tree, side-by-side. Tree and
  ...
  belongs.
- **`references/draw-mockups.md`** — mockups, HTML previews, overlay. A floating pane only exists over a screen,
...
- **"Preferred over a table" was redundant.** `home/CLAUDE.md` line 114 already says *Prefer a list to a
...
- **The glyph probe is a markdown file, not a script.** At `lab/research/ascii-glyph-probe.md`, out of the
