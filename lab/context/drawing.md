...
- **`## Structure` stays in `SKILL.md`.** Ruled twice by the user: first against moving it to a reference file, then against moving it to `home/CLAUDE.md` → `## The reply`, named `## Explaining` at the time. **Do not raise it a third time.**
