...
Confirmed against `lab/research/claude-code-docs/hooks.md` and the skills reference at `code.claude.com/docs/en/slash-commands.md`, 2026-08-22.
...
`lab/framework-build/session.md:132` has an `## On resume` section from the previous generation — read this file, read that one, next action, and a note on what can wait. Same problem, solved by hand.
...
- **`flow open` is the reader** — `scripts/flow/commands/open.js`. Bare it prints the board; an id
