...
- **Where things live** — paths, files, what each holds → a labeled list: the path as the label, one line of what is in it. Never write prose about placement. `## References` in `global/CLAUDE.md` is the model.
...
- **Pick one target, then the step ends** → a `→` list, one line per case, every case covered. `## Capture` in `global/CLAUDE.md` is the model.
...
- **1 line:** "`~/.claude/flow/refs/workflow.md` — how Flow's pieces fit together. Only when that is genuinely unclear". The content moved there intact; the condition became the location.
