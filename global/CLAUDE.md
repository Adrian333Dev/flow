...
- **Any look at structure → `tree.sh`.** Never `ls`, `find`, or `cd` to look around.
  `bash ~/.claude/scripts/tree.sh [path] [--depth N] [--except pattern]`
...
- **More than a few files, or any filtered set → `merge-files.js`.** Query and merge in one call — it replaces grep-then-read whenever the content itself is what's wanted.
  `node ~/.claude/scripts/merge-files.js [--ext ts,tsx] [--except pattern] [--force] <path[:N-M]>...`
...
- **`link-skills.sh`** — re-links every skill in the flow repo into `~/.claude/skills/`. Run after adding, renaming, or removing one; never needed otherwise.
