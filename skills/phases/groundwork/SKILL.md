...
**6 cases change a branch's walk.** When one hits, read its section in `references/walk-cases.md` before answering:

- A branch that can be settled on its own, apart from its siblings → `## When a branch is its own subject`
- A fact the user can't supply: in the code, in documentation, past it, or only found by running something → `## When the user isn't the one who can answer`
- A branch still stuck once the obvious answers are spent → `## When the branch is genuinely hard`
- Layout, feel or a shape at stake, or a proposal, architecture or mechanism going in front of the user for the first time → `## When talking can't answer it`
- New input arriving mid-walk → `## When new input arrives mid-walk`
- Parts, and how they connect → `## When the branch is about structure`
