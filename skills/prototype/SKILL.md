...
**Never write to `map.md`.** Groundwork reads the report and closes its own branch.
...
A prototype covering a whole system answers nothing precisely. A generated-video pipeline has a script step, an image step, a voice step and an assembly step, and each unknown gets its own.
