...
DOM, events, network, rendering — anything that reproduces only inside a page → `/debug-web-pages`. It owns the capture bundles and the probe snippets, and it runs the loop above with tooling built for a page. It builds the failing check; the four steps above still run here.
