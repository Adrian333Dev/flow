...
- **`claude-dir-vs-flow-dir`** `.agents/` holds the one real copy of the rules and the skills. `.claude/` holds what Claude Code reads, and `.flow/` what Flow owns. On the machine, `~/.agents/` carries `AGENTS.md` and `skills/flow/`; `~/.claude/` carries a one-line `CLAUDE.md` importing the rules, `settings.json`, the link `skills/flow`, `agents/` and `rules/`; `~/.flow/` carries `scripts/`, `references/`, `docs/`, `repos/`, `settings.json`, `history.jsonl`, `workflow-notes.md`, `study-cases/` and `private-skills/`, the first 3 being links into the clone. In a project, `AGENTS.md` holds the rules and `CLAUDE.md` imports it; `.claude/` carries `settings.json` and any external skill; `.flow/` carries `tickets/`, `groundwork/`, `inbox.md`, `handoff.md`, `overlays/`, `settings.json` and `settings.local.json`. `flow install` takes one `--root <dir>`, standing in for `~`.
