...
- **`commands/`** — `start.md` and `run.md`. A command earns its place only by running something *before* the model thinks, which is the one thing a skill cannot do: `/start` runs `flow status` and `/start t047` runs `flow t047`, `/run` runs whatever you type. Nothing else belongs here, because a skill is typeable as `/<name>` too — that is why `handoff` became a skill
