...
**Next is the skills build** — the 2 lists, `flow skills`, `flow overlays`, the 4 group folders, and the 10 descriptions rewritten and tried in a real session. `lab/context/design-skills.md` holds it.

**Still unbuilt:** the install skill — one skill covering every starting state, and the last thing Flow gets — and the real migration of the user's own projects. The test harness exists and its coverage is thin: 7 tests over `flow`, `ptree`, `fmerge` and `guard`, which prove the wiring and little else.

**`code-review` is never getting built.** Review runs in the same session, never a subagent, and the criteria live beside whichever skill produced the artifact — `skills/execute/references/review-code.md` is that file for code.
...
One folder per skill under `skills/`; `scripts/link.sh` symlinks each into `~/.claude/skills/`. There is exactly **one copy on the machine** — an edit is live in every project immediately, open sessions included. Flow's own skills are never copied into a project.

To add one: create `<name>/SKILL.md` with `name` and `description` frontmatter, then run `bash scripts/link.sh`. Model-invoked is the default; add `disable-model-invocation: true` to make a skill reachable only when the user types `/<name>`.
...
- **A description says what the skill is and what it covers**, never when to invoke it. A trigger is written only where one is wanted, in 1 of 4 homes — `home/CLAUDE.md` holds the phase routing block. `lab/context/design-skills.md` carries all 4.
- **One skill, one folder, flat today.** The skills build files all 10 into 4 group folders — `phases/`, `tools/`, `standards/`, `stack/` — and `link.sh`'s skill glob widens to `*/*/` on the same day. Until then the glob is `*/` and a group folder would be invisible. Never a vague `misc/`.
...
- **`SKILL.md` is the only file at a skill's root.** Everything else goes in a folder: `scripts/` for executables (`research/scripts/fetch-docs.sh`), `references/` for markdown read on some runs and not others (`visualize/references/draw-mockups.md`), or a purpose name where one fits better (`debug-web-pages/knowledge/`). Set by the user 2026-08-19, and true of every skill since `file-findings/references/write-skills.md` moved 2026-08-24.
...
- **Editing a skill is live inside the running session.** Skills, commands and agents are symlinked into the scratch config, so `SKILL.md` there is this repo's file. Write, save, invoke.
...
- **`home/settings.json`** — permissions, the `PreToolUse` hook, feature flags; every key explained in `home/settings.md`. Merged into `~/.claude/settings.json`
- **`scripts/`** — `ptree.js`, `fmerge.js`, `gsave.sh`, `guard.js`, `snapshot.js`, `link.sh`, `repos.sh`, `try.sh`, `flow/` (`flow.js` is the entry; `lib/` holds the argument layer and the model, `commands/` one file per command group, including `open.js` behind `/start`). Also the Node package root: `package.json` and `tests/`. Symlinked as `~/.claude/scripts`; four get a second symlink in `~/.local/bin` named without the extension, which is what makes `ptree`, `fmerge`, `gsave` and `flow` commands. `guard.js` and `snapshot.js` are hooks, named by path in `settings.json` and never typed
...
- **`commands/`** — `start.md` and `run.md`. A command earns its place only by running something *before* the model thinks, which is the one thing a skill cannot do: `/start` runs `flow open $ARGUMENTS`, which reaches the board, a ticket, a ticket plus a status move, or a loose handoff file; `/run` runs whatever you type. Nothing else belongs here, because a skill is typeable as `/<name>` too — that is why `handoff` became a skill
...
- **A skill or command edit is live immediately.** `~/.claude/skills/*` and `~/.claude/commands/*` symlink into this repo, so there is one copy and no propagation step. Adding, renaming or removing one is the only case needing `bash scripts/link.sh`.
...
- **One source, two ways to reach it.** Every script lives once, in `scripts/`. `~/.claude/scripts` is a symlink to that folder, for files named by path (`guard.js` in `settings.json`, `link.sh`). `~/.local/bin/<name>` are per-file symlinks, for the four that are commands. No file is ever copied anywhere.
...
- **Two languages, by job.** Bash where the script wraps another command (`gsave.sh` over `git`, `link.sh` over `ln`, `repos.sh` over `git clone`, `try.sh` over `ln`); Node where there is real logic (`ptree.js`, `fmerge.js`, `flow/`, `guard.js`). Nothing else.
