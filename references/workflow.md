...
- **Migration**: a change to where Flow and the harnesses keep their files, written only by `/flow:setup-machine`, `/flow:setup-project` and `/flow:migrate`, carried out by a script after one yes, and undone with a snapshot. A ticket is the project's own work, undone with git. Real project work a project setup finds, such as an old `docs/` full of plans, becomes tickets: a migration does only what one yes covers.
