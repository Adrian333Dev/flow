...
- **A group**: `cases`, `domain-skills`, `private-skills`, `overlays`, `git`, `audit`, `snapshot`. A different stored thing, carrying its own actions behind its own name.
- **Setup**: `install` and `doctor`. Neither needs a project: `install` writes outside the project, into `~/.agents`, `~/.claude`, `~/.codex`, `~/.flow` and `~/.local/bin`, and `doctor` reads the same places back. `install` is also the one command run before `flow` is a command at all: on a machine that has just cloned Flow it is typed by path, and it makes the link that lets everything else be typed by name.
...
**A migration has no `flow` command.** `~/.flow/scripts/apply-migration.js <id>` carries one out, run by the skill that wrote it. Bare `flow <verb> <id>` acts on a ticket, so `flow apply <id>` read as a ticket command, and a command on `PATH` can be typed by accident weeks after the migration was written.
