...
function auditRequest(reqJson, realInputTokens) {
  ...
  const systemBytes = reqJson?.system
    ? Buffer.byteLength(JSON.stringify(reqJson.system))
    : 0;
...
function renderAudit(a) {
  const pct = (b) =>
    a.totalBytes ? ((b / a.totalBytes) * 100).toFixed(1) : "0.0";
  const rows = a.toolRows
    .map(
      (r) =>
        `| ${r.name} | ${r.bytes.toLocaleString()} | ~${r.tokens.toLocaleString()} | ${pct(r.bytes)}% |`,
    )
    .join("\n");
...
function printAudit(a, base) {
  ...
  console.log(
    `\n[agent-proxy] ${a.toolCount} tools · ${a.toolsBytes.toLocaleString()} tool bytes` +
      (a.realInputTokens != null
        ? ` · ${a.realInputTokens.toLocaleString()} real input tokens`
        : ""),
  );
  for (const r of top) {
    console.log(
      `  ${r.name.padEnd(w)}  ${String(r.bytes).padStart(7)} B  ~${r.tokens} tok`,
    );
  }
  if (a.toolRows.length > top.length)
    console.log(`  … ${a.toolRows.length - top.length} more`);
...
function renderMessages(messages) {
  ...
  const rendered = messages.map((m, i) =>
    [
      `<message index="${i + 1}" role="${m.role ?? "unknown"}">`,
      "",
  ...
  if (req?.system != null)
    parts.push(
      [
        "<system-prompt>",
        "",
        renderSystem(req.system),
        "",
        "</system-prompt>",
      ].join("\n"),
    );
  if (Array.isArray(req?.tools) && req.tools.length)
    parts.push(renderTools(req.tools));
...
function handle(req, res) {
  ...
  req.on("end", () => {
      ...
      (up) => {
        ...
        up.on("end", () => {
          ...
          try {
            ...
            console.error(
              `[agent-proxy] could not render (non-JSON body?): ${err.message}`,
            );
          }
        });
      },
    ...
    upstream.on("error", (err) => {
      ...
      res.end(
        JSON.stringify({ error: `agent-proxy upstream error: ${err.message}` }),
      );
    });
...
http.createServer(handle).listen(PORT, () => {
  ...
  console.log(
    `[agent-proxy] point Claude Code at it:  ANTHROPIC_BASE_URL=http://localhost:${PORT} claude`,
  );
});
