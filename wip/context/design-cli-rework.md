...
`global/refs/cli-design.md` is the shipped rule set. It was rewritten with this work, and 2 of its rules are reversed: `edit` no longer takes a status, and a command that sets a field is now the normal way to move one.
...
9. **The call sites** — swept across 10 shipped files. Nothing outside `wip/` still writes `flow tickets`, `flow start` or `--status <status>` as a move.
10. **`global/refs/cli-design.md`** — rewritten, 85 → 105 lines. `## One default noun`, `## Status verbs` and `## Print the command, never perform it` are new sections; `## The two kinds of command` became three.
