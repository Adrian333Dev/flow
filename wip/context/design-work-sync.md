...
A new group on `flow`, following `global/refs/cli-design.md`. `edit` is absent: nothing on a stored
copy is editable.
...
`global/refs/work-sync.md` is the usage guide: setup, the two-machine routine, conflicts,
`.flow-include`, what never arrives, and how to recover from a bad restore.

Everything lives in `global/scripts/flow/commands/work.js`, so no new file is linked anywhere. The
...
`node global/scripts/flow/flow.js work …` from this repo.
...
starting `wip:`, and the `wip/` folder already open at backlog line 112.
