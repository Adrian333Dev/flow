...
**`browser-harness`** — a third-party tool from the browser-use team, 16.5k stars, catalogued in `toolbox/browser.md` and cloned at `wip/refs/browser-harness/`. It connects an agent straight to a running Chrome over CDP. The agent clicks, types, navigates and reads the page itself. Roughly 1k lines of core, plus 18 mechanics files and a self-growing store of site-specific knowledge.
