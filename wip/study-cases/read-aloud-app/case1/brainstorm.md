...
latency. **C4 (runtime word alignment) DECISIONS LOCKED 2026-07-21**, now TEST-GATED: mapping =
two joins (normalizer provenance + local token match), never forced-align; cache = ONE simple hash
table (spoken text+voice+speed+model → audio+times), reuse on match incl. duplicate sentences; dropped
junk = zero-duration span. **NEXT = run the 3 C4 tests against the local HeadTTS clone
(`refs/HeadTTS`, node build) to close the branch:** T1 chunk-independence (gates the cache), T2
timestamp/audio desync canary, T3 alignment golden set. After C4 closes → C5 (playback/interaction
...
[ ] C — Reading pipeline: ingest → normalize → segment → streaming synth → sync → seek
    ...
    [~] C4 — Runtime word alignment: DECISIONS LOCKED 2026-07-21 (two-join mapping, never forced-align;
             cache = simple hash(spoken text+voice+speed+model)→audio+times; dropped junk = zero-dur
             span). TEST-GATED to close: T1 chunk-independence (gates cache), T2 timestamp desync
             canary, T3 alignment golden set — run against refs/HeadTTS node build
...
3. **`words` are the ENGINE's tokenization of the SPOKEN (normalized) text, NOT our source words.** This is
   the crux of C4: returned words are post-G2P spoken tokens. Wherever normalization diverges from the
   rendered surface — numbers ("1999"→"nineteen ninety nine"), symbols ("$5"→"five dollars"), abbreviations
   ("Dr."→"Doctor"), stripped markdown — source-word ↔ spoken-word is **N↔M within a segment**, not 1:1.
   Highlighting the rendered document therefore needs an explicit source↔speech alignment (C1 model + C4
   runtime); it is NOT free from `words`. (This is exactly the gap the old voiceover spec glossed over.)
...
**Architectural catch (affects C1/C4):** HeadTTS normalizes **internally**, so its returned `words` are
already-expanded spoken words with NO per-word link back to our source tokens — letting it normalize loses
provenance for highlighting/alignment. ⇒ Strong lean: **do normalization in OUR pipeline** (we know
"$5"→["five","dollars"] and keep the source→spoken map), feeding HeadTTS pre-normalized text or typed items.
Confirm mechanics in C4 (whether returned `words`/subtitles suffice to map per-word).
...
**Highlight-to-audio mapping = TWO JOINS, never forced-alignment** (research: purpose-built aligners
miss the acceptable-drift window on 30%+ of sentences; Whisper-align 6.6% → forced-align is out; use
HeadTTS synthesis-time timestamps only).
- Join 1 (source token → spoken tokens): RECORDED by the C2 normalizer as it transforms text
  ("$5" → "five dollars"). Exact, free — it's the normalizer's own output.
- Join 2 (our spoken tokens → HeadTTS returned `words[]` + `wtimes`): trivial LOCAL text match within
  the chunk; fallback if it can't line up = distribute the chunk's time proportionally across the
  affected source tokens (still no forced-align).
- Compose → each source token gets a `[start,end]` span RELATIVE to chunk start (+ chunk base-offset =
  absolute, per the caching timeline split). Worked ex. "I paid $5 today." → "$5" span = [start of
  "five", end of "dollars"]; click "$5" → seek to that start.
