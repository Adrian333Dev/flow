...
**Where we are:** ALL branches resolved — A, E, B, C (C1–C5), and **D formally closed 2026-07-23**.
Remaining: Phase 3 assumption check (written to `## Assumptions`, awaiting user reaction) → Phase 4
spec. Engine is
**English-only** (HeadTTS/Kokoro) → multilingual out of scope for v1. Deferred within otherwise-done
branches: B5 (warmup UX), A4 (name).
...
(currency/symbols/abbrev). C2/C4 reconciled.** **C5 FOUNDATION LOCKED (2026-07-23, user confirmed
"fully makes sense"):** UI layer + playback controller over Web Audio, single clock =
`audioContext.currentTime`, controller writes `activeWordIndex` — full record in "Branch C5 —
foundation" below. **C5b LOCKED (2026-07-23):** buffering third state + immediate highlight ack +
queue re-anchor on seek (pending synth dropped, in-flight completes + caches); parallel-synth idea
captured as deferred optimization. **C5c LOCKED (2026-07-23):** model-level speed, re-synth forward
from the current word, applied immediately; speed in the cache key. **C5d LOCKED (2026-07-23):**
pause = `audioContext.suspend()`/`resume()`; synthesis continues during pause. **C5e LOCKED
(2026-07-23):** pre-warm at app load (+ dummy synth), eager FIRST-chunk-only synth on paste,
play-before-ready = C5b cold seek. **BRANCH C FULLY RESOLVED (C1–C5). BRANCH D FORMALLY CLOSED
(2026-07-23)** — persistent+layered cache confirmed; key = exact synth-input string + voice/speed/
model; v1 = IndexedDB tier behind `CacheStore`; caps = benchmark knobs (see "Branch D: Caching —
FORMAL CLOSE"). **ALL BRANCHES RESOLVED** (deferred: A4 name, B5 warmup UX — see "## Deferred").
**Phase 3 assumptions written (`## Assumptions`). NEXT = Phase 4: write spec.md** once the user
reacts to the assumption check. C5 inherits the runtime highlight policy (never lag audio; advance to next word
whose start ≥ currentTime; ~150ms-ahead OK, ~50ms lag perceptible). (Note: memory is retired — all
standing rules now live in the repo root `CLAUDE.md`. 2026-07-23: ALL skills are global under
`~/.claude/skills/`; explanations go through the global `explain` skill — the old local
`visualization` skill and this topic's `visualizations/` files were deleted.)

**Also (2026-07-21, this session):** full pipeline architecture visualized for the user (the
`visualizations/` files were deleted 2026-07-23; explanations now use the global `explain` skill).
Clarified for the user: HeadTTS's real
...
[x] C — Reading pipeline (RESOLVED 2026-07-23 — C1–C5 all locked): ingest → normalize → segment →
        streaming synth → sync → seek
    ...
    [x] C2 — Format scope + normalization (RESOLVED 2026-07-21): markdown cleaner in v1 (code block →
             label + skip; tables → cell-by-cell); unknown-token fallback (symbol-name table / silent
             zero-duration span / G2P for word-shaped tokens); structural pre-clean for PDF/web paste;
             reconciled 2026-07-22 with C4 Option B (normalizer = targeted override only)
    ...
    [x] C5 — Playback & interaction controller (RESOLVED 2026-07-23 — C5a–C5e all locked)
        [x] C5a — Foundation: UI layer (React) + playback controller (plain JS over Web Audio API);
                  single clock = audioContext.currentTime; controller writes activeWordIndex;
                  commands down (play/pause/seekToWord/setSpeed) / position up (setActiveWord)
        [x] C5b — Cold-region behavior (LOCKED 2026-07-23): buffering third state pushed up to UI;
                  highlight jumps to clicked word immediately; seek re-anchors the C3 queue —
                  pending synth dropped, in-flight completes + caches; parallel-synth idea
                  captured as deferred benchmark-gated optimization
        [x] C5c — Speed-change mechanics (LOCKED 2026-07-23): model-level Kokoro speed, re-synth
                  forward from the current word, applied immediately (reuses C5b buffering path);
                  speed in the cache key; playbackRate rejected (pitch shift, no preservesPitch
                  on Web Audio buffer sources)
        [x] C5d — Pause/resume mechanics (LOCKED 2026-07-23): pause = audioContext.suspend(),
                  resume = .resume() — freezes the single clock, sample-exact continue, zero
                  bookkeeping; synthesis queue keeps filling during pause
        [x] C5e — First-audio behavior (LOCKED 2026-07-23): pre-warm at app load (init + dummy synth
                  absorbs WebGPU shader compile); on paste eagerly synth the FIRST chunk only;
                  play-before-ready = cold seek to word 0 (C5b); cold-load UX remains B5's
[x] D — Caching & indexing (FORMAL CLOSE 2026-07-23): 2026-07-21 persistent+layered+universal-key
        design confirmed; key = exact synth-input string + voice/speed/model version; v1 = IndexedDB
        tier behind CacheStore; caps = benchmark knobs; read-ahead prefetch = C3 look-ahead (no new
        machinery)
```

## Assumptions (Phase 3 check, 2026-07-23)

The bets the design rests on, unverified as of close:

1. **Warm TTFA of 300–750 ms on real consumer GPUs in-browser.** Source = external benchmarks
   (QuickTTS table, B3), not our own measurement. If wrong on target hardware → the ~1 s
   first-audio promise slips; rethink chunk-length tuning + eager-synth scope, lean harder on
   daemon/hosted. Mitigation already planned: the deferred real-hardware/low-spec TTFA benchmark
   BEFORE v1 ships.
2. **HeadTTS's verbatim-source `words[]` contract holds beyond the tested env.** T3 proved it on
   HeadTTS v1.3.0, Node REST, q8/CPU. Option B's free 1:1 alignment rests on the browser build and
   future versions behaving the same. If broken → the retained two-join fallback machinery covers
   it; pin the dependency version and re-run T3 on every upgrade (cheap tts-lab run).
3. **Clause-seam prosody is acceptable.** C3's clause-splitting of long sentences bets Kokoro
   sounds natural across intra-sentence chunk seams. If seams are audible → fallback already
   recorded: whole-sentence-always + a short-first-chunk exception for TTFA.
4. **In-browser Opus encoding is available** (WebCodecs `AudioEncoder`, or a small WASM encoder as
   fallback). If not → store Int16/raw PCM in IndexedDB (4–8× bigger, caps hit sooner); the design
   is otherwise unchanged.

## Deferred / out of scope (confirmed at close)

- B5 — cold-start/warmup UX for the 7–18 s first-ever model load (own session).
- A4 — product name & identity.
- Real-hardware / low-spec TTFA benchmark (pre-v1 gate; assumption #1's mitigation).
- Parallel chunk synthesis (C5b captured optimization; benchmark-gated, never default-on).
- Hosted shared-cache timing-observability privacy call (before hosted ships).
- Verify what the audio actually SPEAKS for "$5" + the typed-items override path (cheap tts-lab
  check; C4 non-blocking caveat).
- v2 formats: LaTeX / nested structures / HTML / footnotes (C2).
...
### Branch C5 — playback & interaction controller — FOUNDATION LOCKED (C5a, 2026-07-23)

**Decision (user confirmed the proposal "fully makes sense to me"):** the client side splits into two
pieces.

- **UI layer (React).** Renders the word list + transport controls. State: `words[]` +
  `activeWordIndex`; the word at `activeWordIndex` gets the highlight style. It never computes
  timing — it only renders state and forwards user intent.
- **Playback controller (plain JS/TS module, no React).** Owns the per-chunk audio clips + their
  word-timing data (HeadTTS `wtimes`) and plays them through the **Web Audio API**. Web Audio over an
  `<audio>` element for three load-bearing abilities: (1) an exact clock —
  `audioContext.currentTime`, the SINGLE time source for everything; (2) gapless back-to-back
  scheduling of consecutive clips; (3) starting a clip at an arbitrary millisecond offset
  (word-level click-to-seek needs this).

**The one non-obvious rule:** the CONTROLLER writes `activeWordIndex`, not the UI. A UI-side timer
(setInterval) drifts from real audio within tens of seconds; the controller instead reads the audio
clock every frame (rAF loop), maps clock → current word via the timing data, and pushes the index up.
...
**Locked mechanics (from the accepted walkthroughs):**
- Play: controller schedules clip k then k+1 back-to-back on the audio clock; every frame: read
  clock → timing data → `setActiveWord(n)` → UI re-renders with that word highlighted.
- Click word 47: `seekToWord(47)` → timing data resolves word 47 = clip 3 @ offset 14.8s → stop the
  current source, start clip 3 at that offset → `setActiveWord(47)`.

**Inherited constraints:** highlight never lags audio (advance to the next word whose start ≥
currentTime; ~150ms early imperceptible, ~50ms lag perceptible — C4); stop highlight at lastWordEnd,
not audio end (8–17% trailing silence — T2); exactly one chunk per synthesize call (T1).

**Rejected:** `<audio>` element (no precise shared clock, no gapless chaining, no reliable
sub-second offset start); UI-owned setInterval highlight loop (drift); letting React/UI state drive
audio (audio is the master clock; the UI is a follower).

**All C5 sub-branches resolved (C5a–C5e).**

### Branch C5b — cold-region behavior — LOCKED (2026-07-23)

**Decision:** the controller gets a third state — `buffering` — alongside playing/paused, pushed up
to the UI as a second upward signal (`setPlaybackState('playing'|'paused'|'buffering')`). When play
or a seek targets a COLD chunk (no clip in the cache):

1. `setActiveWord(n)` fires IMMEDIATELY on the click — the highlight jumps within a frame so the
   click feels registered — plus `setPlaybackState('buffering')` (UI free to show a spinner; audio
   stays silent). User approved immediate-jump over highlight-waits-for-audio.
2. The target chunk jumps to the FRONT of the C3 priority queue; the look-ahead window re-anchors
   around the new position (the chunks after the target become the next synthesized).
3. When the clip arrives, the controller starts it at the word's in-chunk offset →
   `setPlaybackState('playing')`.

Warm-GPU chunk synth ≈ 300–750 ms → most cold seeks are a sub-second blip; WASM fallback can be
seconds — exactly why buffering must be a VISIBLE state, not silent dead air.

**Cancellation semantics (user question 2026-07-23, answered + locked):** on a new seek, everything
QUEUED but not yet started is dropped/re-prioritized immediately — the old look-ahead is abandoned,
and a superseded mid-buffer target loses its priority. The ONE chunk mid-inference is NOT aborted:
ONNX runtime can't usefully cancel a running inference, so it runs to completion (~sub-second warm)
and its result is CACHED (the work is already paid for; the user may come back to that region).
Worst-case extra latency for the new target ≈ one chunk's synth time — one more reason C3 keeps
chunks short.

**Rejected:** synthesizing strictly in document order and making the click wait (clicking the last
paragraph of a long doc would wait for the whole doc); silent buffering (indistinguishable from a
dead click); UI inferring buffering from missing position updates (fragile — hence the explicit
state signal).

**Captured optimization (user idea 2026-07-23, DEFERRED — benchmark-gated, never default-on):**
parallel chunk synthesis on capable machines (first chunk immediately, then e.g. chunks 2–4 in
parallel). Caveats recorded now: parallelism = multiple model instances (each worker duplicates
model memory, ~hundreds of MB); one WebGPU inference already saturates most GPUs, so parallel runs
would likely SLOW the first chunk (worse TTFA); ONNX's WASM backend already multi-threads one
inference across cores; and serial look-ahead on GPU is already ~3× realtime, staying ahead
naturally. The plausible win is multi-core CPU machines on the WASM path. Needs the extensive
testing the user named; belongs with the C3 queue implementation + the deferred low-spec benchmark.

### Branch C5c — speed-change mechanics — LOCKED (2026-07-23)

**Decision:** speed is MODEL-LEVEL — Kokoro's speed parameter re-synthesizes the speech at the new
rate (correct pitch, natural rhythm). On a speed change the controller re-synthesizes FORWARD from
the current word and applies it IMMEDIATELY (user: "apply it immediately, without waiting"): the
current chunk at the new speed jumps to the front of the queue, the C5b `buffering` state covers
the swap (sub-second warm), and the look-ahead continues at the new speed. **Speed is part of the
cache key** (re-affirms C4's `hash(chunk text + voice + speed + model)`) — a chunk only counts as
"already generated" if it was generated AT THIS SPEED (user confirmed). Old-speed clips stay
cached, so switching back is instant for already-synthesized regions.

**Rejected:** Web Audio `playbackRate` (user: "doesn't make sense") — buffer sources have no
pitch-preserving mode, so playback pitches up; shipping a time-stretch DSP (phase vocoder) to fix
it is far too heavy for v1. (Timestamp scaling was NOT the problem — wtimes would scale linearly
with the rate; pitch is the killer.) Also rejected: lazy application (finish the current sentence
at the old speed, apply from the next chunk) — zero-blip, but the control lags seconds behind the
click and feels ignored.

### Branch C5d — pause/resume mechanics — LOCKED (2026-07-23)

**Decision:** pause = `audioContext.suspend()`, resume = `audioContext.resume()`. Suspending freezes
the ENTIRE audio context — the clock stops ticking, everything already scheduled stays scheduled —
and resume continues sample-exact from the frozen instant. Because the whole C5 design reads the one
clock (`audioContext.currentTime`), freezing it pauses every part consistently for free: the rAF
loop reads a frozen clock so the highlight simply stops moving; queued next clips stay queued;
there is no position to remember. Controller state → `paused`.

**Rider (locked):** the synthesis queue KEEPS WORKING while paused — pause frees compute, so the
look-ahead keeps filling the cache; resume returns to a deeper buffer, and a seek-while-paused is
more likely to land on warm audio. Memory stays bounded by the C3 look-ahead window either way.

**Rejected:** stop-and-remember-offset (stop the source, store the position, re-run the seek path
on resume) — manual bookkeeping, tears down and re-schedules the queued clips, and risks tiny gaps
or off-by-a-few-ms errors at the seam. Suspend/resume is two lines with nothing to get wrong.

### Branch C5e — first-audio behavior — LOCKED (2026-07-23)

**Decision:** spend the user's idle time before they press play. Three moves:
1. **Pre-warm at app load:** start model init the moment the app loads — never wait for the play
   click — including a tiny throwaway synth: the first WebGPU inference also compiles shaders, so
   the dummy run absorbs that one-time cost and the first REAL synth hits the warm 300–750 ms path
   (B3 technique, now locked as controller behavior).
2. **Eager first chunk on paste:** the instant text lands, segment it (cheap/local, C3) and
   synthesize the FIRST CHUNK ONLY in the background — the seconds the user spends glancing at the
   text are enough to have clip 1 cached → press play = instant sound.
3. **Play before it's ready = nothing special:** that's a cold seek to word 0; C5b's buffering
   state covers it.

**Eager scope = first chunk only (locked):** it's the only chunk whose latency the user ever feels
(once playing, look-ahead outruns realtime); synthesizing more up-front burns compute/battery on a
document that may never be played or gets edited first. A wasted eager chunk is cheap anyway:
content-addressed cache — an edit elsewhere leaves chunk 1's hash unchanged → reused.

**Boundary:** the 7–18 s first-ever-visit cold model download/init UX stays B5's (own session).
C5e locks only controller/queue behavior: init early, dummy-synth to warm, eager chunk 1 on paste.

**BRANCH C5 FULLY RESOLVED (C5a–C5e) — and with it Branch C (C1–C5).**

## Branch D: Caching & indexing — FORMAL CLOSE (2026-07-23)

The substance was user-decided 2026-07-21 (see "Branch D cache — PERSISTENT + LAYERED + UNIVERSAL
KEY" block above): persistent layered cache, one universal content key, `CacheStore { get, set }`
implemented per environment (browser → IndexedDB, daemon → disk store, hosted → server store),
read-through + write-back, encoded Opus + per-word timings JSON, LRU by bytes per tier, model
version inside the key. Formal close confirms that design and locks four reconciliations (user
confirmed 2026-07-23):

1. **Key text input reconciled with Option B (C4):** key =
   `hash(EXACT STRING SENT TO THE SYNTHESIZER + voice + speed + model version)` — source chunk text
   by default, with the targeted overrides applied on TN-gap spans. (The old "normalized text"
   wording predates Option B.) This is precisely the key T1 proved safe.
2. **v1 scope = ONE tier:** phase 1 ships only the in-browser IndexedDB store, but behind the
   `CacheStore` interface from day one; daemon/hosted stores arrive with their providers (phases
   2/3) with zero core changes.
3. **Cap numbers = benchmark-tuned knobs, not design:** byte caps / eviction thresholds get tuned
   alongside C3's chunk-length number. The design fixes only the shape: LRU by bytes, per-tier cap.
4. **Bounded RAM by construction:** decoded PCM exists ONLY inside the C3 look-ahead window;
   everything else lives as small Opus blobs in IndexedDB, decoded on approach. Document length
   never inflates memory.

Read-ahead prefetch (the other half of D's original title) needs nothing new — it IS C3's adaptive
look-ahead plus C5b's re-anchoring priority queue.

Noted as assumptions, not branches: in-browser Opus ENCODING needs WebCodecs `AudioEncoder` (WASM
encoder or raw-PCM storage as fallbacks); different backends may produce different audio bytes for
the same key — harmless, any tier's rendering of that text is valid. Deferred (pre-hosted-launch):
the shared-hosted-cache timing-observability privacy call.
