...
- [ ] **A dropped file path costs a whole extra turn**: dragging a file from the editor into the terminal pastes its absolute path in single quotes, and dragging is the only easy way to name a file the `@` list will not offer, a git-ignored one above all. The agent then spends one turn seeing the path and a second reading the file. **Wanted: the drop inserts `@path`**, which the user set on 2026-09-21 as the only acceptable outcome. **Researched the same day, and it is 2 faults in 2 different places.**
  - **The list of suggested paths is ours to replace, and that half is buildable now.** `fileSuggestion` in `settings.json` names a script of ours, handed `{"query": "src/comp"}` on stdin and answering with up to 15 paths, so a git-ignored file gets offered. The user asked for 2 things from it: most recently changed first, and an ignore list for what no answer ever wants, `node_modules/`, `dist/`, `.git/` and build output. **This is the only piece of the drop problem Flow still builds.** **talk first**
  ...
  - **Filed rather than built**: 3 `[FEATURE]` issues on 2026-09-21, the file drop, the folder drop, and a hook being able to replace a prompt. `claude-code.md` → `### Three more on 2026-09-21, written from the dropped-path research` holds all 3, and `### The feature form, field by field` beside it holds what the form asks for.
