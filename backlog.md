...
Routed out of `lab/context/management.md` on 2026-09-17, the last phase of the groundwork method. **This list runs in the order written.** Nothing here is designed further: each line has a dated section in that map holding the argument, and the map stays as the design record every line points back to. The 3 skills come last, because each one calls code that does not exist yet.

- [ ] **The 2 sweeps over the repo run first**, since the new skills are written into the tree they leave behind: the `(user invoked)` mark and the `references/style.md` split, each with its own line under `### The skill system` and `### Rules and always-loaded files` above. In that order they buy a typed-only set the 3 new skills join the day they are written, and `references/write-docs.md` carrying the rule that a manual page added, renamed or dropped updates `docs/manual/README.md` in the same edit, which is the index `/flow:help` reads

- [ ] **`flow snapshot new|ls|restore`**: copy every file a run may change into `~/.flow/snapshots/<date-time>/` before the run starts, beside a manifest saying what was copied and what the run intended. A symlink is a row holding its path and its target, never a copy, or the copy follows the link. `restore` puts the files back, re-points the links and deletes what the run created, from a plain shell with no session open. The run's `plan.md` lands in the same folder. Every line below needs it. `management.md` → `## Snapshot and restore`

- [ ] **`flow install`, 5 changes in one pass over `scripts/flow/commands/install.js`.** It stops writing `~/.claude/CLAUDE.md` at all, because `install.js:147` asks only whether the file exists and this machine's is 0 bytes, and the rule file now lands in `/flow:setup` after the interview. Its closing message at `install.js:156-166` becomes "restart Claude Code and type `/flow:setup`" in place of a hand merge of `home/settings.json`. It prunes any link in `~/.local/bin` pointing into the clone whose name has left the `BIN` map at `install.js:55`, which nothing detects today because such a link still resolves. It links `~/.flow/docs` to the clone's `docs/`, so a skill names the manual by a fixed path rather than `~/.flow/scripts/../docs/`. And on a machine where `~/.flow/snapshots/` is empty it writes the manifest of what it is about to create before creating it, so restore reaches the bootstrap links too. `management.md` → `## What a machine installs`, `### Setup opens in a terminal, not a session`, `### A stale name in ~/.local/bin`

- [ ] **`CHANGELOG.md` comes back, and it is the whole input to a migration.** `no-changelog` suspended it on 2026-08-09, and `CLAUDE.md` says it returns at the first release. The design now needs it for more than history: `~/.flow/version` is one line holding the date of the newest entry a machine applied, step 2 of a migration reads every entry newer than that date, `flow doctor` compares the two, and the git tag a machine installs from carries the same date as its name. **Nothing below this line can ship while the file does not exist.** What has to be settled: what an entry holds for a migration to act on, beyond today's rule of one line per change of behavior; and whether an entry may say "the old skill cannot carry this out", which `## When a migration changes the management skill itself` leans on. **talk first**, the suspension is the user's to lift. `management.md` → `## The version`

- [ ] **`flow doctor` gains the version checks.** `~/.flow/run.json` on disk is reported before anything else as a problem, exit non-zero, naming the step the run stopped at and the 2 ways out. Three comparisons are notes with `flow up` suggested and exit 0: the machine's `~/.flow/version` against the newest changelog entry, a project's `.flow/version` against the machine's, and the clone against the newest tag, that last one behind `flow doctor --updates` because it is the only check that touches the network. A `~/.flow/version` newer than the newest entry is a problem, since the clone moved backwards. A submodule sitting off its gitlink is a note, never a problem. `management.md` → `## The version`, `## Prerequisites are checked by running them`

- [ ] **`~/.flow/settings.json` gets a switch per thing Flow prints, and the reminder becomes `reminder.js`.** The `UserPromptSubmit` hook runs `cat` over `references/reminder.md` today, and a bare `cat` cannot read a setting, so the hook line becomes `node "$HOME/.flow/scripts/reminder.js"` and `"reminder": false` silences it. `management.md` → `## The check that runs by itself is a hook`

- [ ] **A `SessionStart` hook prints one line when the machine or the project needs attention**, reading `~/.flow/version`, the project's `.flow/version` and `~/.flow/run.json`, silent when all 3 are fine, behind its own switch. `home/settings.json` carries no `SessionStart` hook today. This is also what names `/flow:migrate` to a user who cannot see the skill, since `disable-model-invocation: true` keeps its description out of every session. `management.md` → `## The check that runs by itself is a hook`, `### All 3 are typed, never model-invoked`

- [ ] **The domain-skills clone pulls itself, and one of its skills may go global.** The same `SessionStart` hook starts a detached `git pull` in `lab/domain-skills` and returns at once, only when the clone's `FETCH_HEAD` is hours old, and refuses on either of 2 guards: uncommitted changes there, or a pull that would not be a fast-forward. `"domainSkillsAutoUpdate": false` turns the pull into a fetch that writes what is behind into `~/.flow/skills-update.json` and prints the skill names every session until the user pulls. `flow domain-skills add <name> --global` links a skill into `~/.claude/skills/` and lists it in `~/.flow/domain-skills.txt` so a second machine gets it back, saying at add time that its description then loads in every session on the machine; the code refuses this outright today. `management.md` → `## Domain skills`

- [ ] **`references/prerequisites.md`**: step 0 of both setup and migration, read by each rather than invoked, because it never runs on its own. It checks the commands Flow calls by running them, `util fs tree`, `util fs merge` and `util fs open`, never by comparing a submodule commit; it finds the domain-skills clone through `domainSkills` in `~/.flow/settings.json` and checks nothing else about it, since Flow never calls it; and it says the toolbox is temporary and gets replaced whole. `management.md` → `## Prerequisites are checked by running them`

- [ ] **`skills/tools/setup/SKILL.md`**: put Flow on a machine that never had it, or that has somebody else's setup on it. The survey reads all 4 routes a skill arrives by and looks for Flow's own leftovers first. The competitor test asks whether a thing instructs behavior on ground Flow already rules on, with a third outcome beside keep and delete for the account-synced tree that comes back on the next sync. The harvest sorts every line into 3 buckets and drops one only where the plan names the Flow rule replacing it. The interview asks what the harvest left blank and fills `## The user` and `## Preferences`. `settings.json` splits 3 ways, with the 7 opinion keys asked once each and every other key left exactly as it is. Then the rule file is written. The project half is the same run inside a project, the first time the user opens it. One plan, one yes, then to the end. **The interview's question list is its own design pass, inside this build.** `management.md` → `## Setup and migration are two flows`, `## The competitor test`, `## Harvesting an existing setup`, `## What setup does to settings.json and to a project`, `## The 2 personalised files`

- [ ] **`skills/tools/migrate/SKILL.md`**: 6 fixed steps, each naming the file it reads or writes. Snapshot; read `~/.flow/version` and every newer changelog entry; diff the 3 things Flow owns; write `plan.md`, one line per file rather than one per entry; apply, with step 5 writing Flow's own files whole rather than patching lines; prove with `flow doctor` and one `claude --print` session read back through `flow audit`, then stamp the version. The session that applied it holds rules that are gone, because rules load when a session launches, which is why the proof runs in a second process. `management.md` → `## The 6 steps of a migration`, `## A whole-workflow replacement`, `## Proof`
...
- [ ] **`flow up`**: the whole update in one word. Fetch, see what is newer, update the submodules, run the migration, run `flow doctor`. Inside a project it does the same against `.flow/version`. `management.md` → `## How Flow ships`
...
- [ ] **`flow skills ls` prints two columns that can no longer say anything.** `STATE` and `SET BY` read `skillOverrides`, which stopped reaching Flow's skills when they became a plugin on 2026-09-18, so every row now says `on` and `default`. The cut: drop both columns and the `--hidden` flag, which filtered on `STATE` and now means nothing but `drafts/`; drop `states()` and `readOverrides()` from `scripts/flow/lib/skills.js` once nothing calls them. `docs/manual/reference.md` → `### flow skills ls` and `docs/manual/settings.md` → `### skillOverrides` both say the columns are going and update in the same pass. What holds it back is nothing but a decision on what `ls` should print instead: group and install state, or the catalog alone

- [ ] **A typed-only skill is marked `(user invoked)` on first mention** in any file the agent reads. `disable-model-invocation: true` takes the description out of context, so prose is the only place the model meets the skill, and prose reads like something it can run. One live fault: `skills/phases/groundwork/references/write-spec.md` line 119 says **invoke `/flow:cut-from-spec`**, which the agent cannot do. Today's set is `/flow:start`, `/flow:cut-from-spec` and `/flow:fold`. The convention goes in `references/style.md`, and one line in `home/CLAUDE.md` under `## Workflow` lists the set. Word chosen by the user 2026-09-17
...
- [ ] **`references/style.md` splits by job.** 244 lines and 11 sections covering three different jobs, and `writing-pass` sends every markdown edit through all of it, so writing a manual page means reading the rule-id rules. The cut: `style.md` keeps what every file needs, plan the shape first, section shapes, one home per fact, branching a step, sentences, words, never cut these, transformations; `references/write-rules.md` takes §11 rule ids; `references/write-docs.md` takes §10 a documentation page plus the rule that a new, renamed or dropped page updates `docs/manual/README.md` in the same edit. The rule in both `CLAUDE.md` files then names which of the three per job. Raised by the user 2026-09-17
...
- [ ] **Flow on another harness and on another model.** Researched 2026-09-06 and 2026-09-07, and nothing is locked. **The user wants the Codex half sooner, 2026-09-17**: either while the management skill is being built or straight after, because testing that skill means testing it against Codex too. That part may belong in `## V1`. Building it for a stranger split off into `## V1` on 2026-09-16. **talk first**. `models.md`. The parts:
