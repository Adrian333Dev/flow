...
- [ ] **The management skill**: Flow's whole life on a machine, and the last thing Flow gets. **Far larger than an installer**, decided 2026-09-01. It is 4 entries the user types: the commands `flow setup`, `flow setup project` and `flow up`, and the skill `/flow:help` in `skills/tools/`. It also carries the live checks `flow doctor` cannot make: whether a hook fires, whether a rule holds under load. **The groundwork is finished.** The map was walked to the end, attacked against this machine, and routed on 2026-09-17, so the work is now a list in one order: `### The management skill, in build order` below. This line is the spine's marker for it and holds nothing the order does not. `management.md`
...
- [ ] **`changes.test.js` → *2 workers at once each get only their own file* fails about 1 whole-suite run in 8.** Never alone: 36 runs of its file in isolation passed. The failing record lists every file, the worker's own edit included, under *changed by no tool call a hook saw*. So the safety net in `buildRecord()` found no event ending after the worker's `start.json` time, which points at timestamps compared across processes and is unproven. The other flaky case, *a worker hands the parent its diff*, was a real bug, fixed 2026-09-26: `snapshot()` copied git's index with a fresh time, which turned off git's check for an edit of the same size in the same second as the commit.
...
- [ ] **`flow setup project`**: bring one project into Flow, the first time the user opens it. **Built 2026-09-26, never run against a real project.** What is left is the run: Delapse in the sandbox first, then the fixes it finds. `lab/context/state.md` has what is built, and `management.md` → `## Project setup is a command, ruled 2026-09-25` every decision.

- [ ] **`flow up`**: the whole update in one word, for the machine and for the project it is typed in. **Built 2026-09-26, never run on a real update**: the first real one is entry 2, the first change after Flow is installed. It pulls the clone and its submodules, then opens a Claude Code session per place that is behind, which reads the upgrade guides and writes one form. A session rather than the `/flow:migrate` skill, ruled 2026-09-26, so the newest steps always run. No run has shown whether `flow audit session` lists the hooks a `claude -p` session fired, and the proof step relies on it. `lab/context/state.md` has what is built, `management.md` → `## Migration is a session flow up opens, ruled 2026-09-26`

- [ ] **`skills/tools/help/SKILL.md`**, **parked** until the manual is rewritten after the final sweep, ruled 2026-09-26: its answers are only as good as the pages it names. Answer a question about Flow in one line plus an address, such as `docs/manual/settings.md` → `## Hooks`, found through `~/.flow/docs/manual/README.md`. "What do I do now" is state rather than documentation: read `~/.flow/run.json`, then the board, then `.flow/handoff.md`, and name `/flow:start` instead of rebuilding what it renders. `management.md` → `## The answer job`
...
- [ ] **Every command's output needs a clean terminal layout.** Tested 2026-09-26 on `flow restore machine` with 2 projects: the options, the fallback command and the paths ran together, and the first line read "Puts 1 paths". A machine with dozens of paths would be unreadable. Applies to every command that prints, the help and any instructions first. Raised by the user 2026-09-26.
...
- [ ] **A second machine cannot join `~/.flow/`'s repository.** Found 2026-09-26 testing `flow sync` against a bare repository: the first pull on the second machine refuses, because the files its own install and setup wrote, `.gitignore` and `AGENTS.md` among them, sit where the pull would write. Open: whether `flow install` brings the repository down at its question, before setup runs, and whether setup then keeps the `## The user` and `## Preferences` that arrived. **talk first**

- [ ] **A session opened in a repository Flow was never set up in says nothing.** The session check stays silent outside a folder holding `.flow/`, and the machine setup's last message never names `flow setup project`. Not every repository is a project: `~/code/playground` and `~/kb_v0` are both git repositories. Raised by the user 2026-09-26, who wants the suggestion only where the folder looks like a project. **talk first**
...
- [ ] **A dropped file path costs a whole extra turn**: dragging a file from the editor into the terminal pastes its absolute path in single quotes, and dragging is the only easy way to name a file the `@` list will not offer, a git-ignored one above all. The agent then spends one turn seeing the path and a second reading the file. **Wanted: the drop inserts `@path`**, which the user set on 2026-09-21 as the only acceptable outcome. **Researched the same day, and it is 2 faults in 2 different places.**
  ...
  - `lab/research/claude-code-docs/hooks.md`, `settings-reference.md` → `### fileSuggestion`
...
- [ ] **Run `/doctor prompt-audit` on this repo once.** Added in Claude Code 2.1.283, read off its changelog 2026-09-26. It reads every `CLAUDE.md`, skill, agent and command, and lists stale paths, stale commands and instruction files that contradict each other before wording written for older models. Take the report as a list of references to check, and refuse every rewrite it offers: its idea of old wording may break `references/style.md`. The docs describe no `prompt-audit` yet.
...
- [ ] **Tune `guard.js`'s deny and ask lists** against real use: they were written from the rules, never against an observed false positive **parked** until Flow is in daily use
