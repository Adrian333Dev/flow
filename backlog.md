...
- [ ] **`flow install`, 5 changes in one pass over `scripts/flow/commands/install.js`.** It stops writing the rule file and its 2 ways in, `~/.agents/AGENTS.md`, the import line in `~/.claude/CLAUDE.md` and the link `~/.codex/AGENTS.md`, because the rule file now lands in `/flow:setup` after the interview. Its closing message becomes "restart Claude Code and type `/flow:setup`" in place of a hand merge of `home/settings.json`. It prunes any link in `~/.local/bin` pointing into the clone whose name has left the `BIN` map in `install.js`, which nothing detects today because such a link still resolves. It links `~/.flow/docs` to the clone's `docs/`, so a skill names the manual by a fixed path rather than `~/.flow/scripts/../docs/`. And on a machine where `~/.flow/snapshots/` is empty it writes the manifest of what it is about to create before creating it, so restore reaches the bootstrap links too. `management.md` → `## What a machine installs`, `### Setup opens in a terminal, not a session`, `### A stale name in ~/.local/bin`
...
- [ ] **A rule id is written by hand and nothing checks the naming.** The convention lives in `references/style.md` §11, and `flow scorecard` reports an id defined twice in one file. Left open: whether the same rule in `home/AGENTS.md` and the repo `CLAUDE.md` must keep the same id, which is what makes `flow scorecard` count it once. **parked** until rules are added from mining rather than by rewriting
...
- [ ] **Flow on another harness and on another model.** Researched 2026-09-06 and 2026-09-07, and nothing is locked. **Codex waits until Flow has shipped on Claude Code**, ruled by the user 2026-09-18, reversing a ruling from the same morning that put the port before the management skill. Each harness may get its own mechanisms rather than one bent to fit both. The `flow:` prefix and the one real copy of each file in `~/.agents/` were built before the reversal and stay, since any harness needs them. Building it for a stranger split off into `## V1` on 2026-09-16. **talk first**. `models.md`. The parts:
  ...
  - **A skill linked onto the whole machine never reaches Codex.** `flow private-skills add --global` links into `~/.claude/skills/`, and the planned `flow domain-skills add --global` would too. Codex reads `~/.agents/skills/` alone, so both would move there, with a link back for Claude Code the way Flow's own plugin folder has one
