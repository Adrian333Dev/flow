...
- [ ] **`flow install`, 5 changes in one pass over `scripts/flow/commands/install.js`.** It stops writing the rule file and its 2 ways in, `~/.agents/AGENTS.md`, the import line in `~/.claude/CLAUDE.md` and the link `~/.codex/AGENTS.md`, because the rule file now lands in `/flow:setup-machine` after the interview. Its closing message becomes "restart Claude Code and type `/flow:setup-machine`" in place of a hand merge of `home/settings.json`. It prunes any link in `~/.local/bin` pointing into the clone whose name has left the `BIN` map in `install.js`, which nothing detects today because such a link still resolves. It links `~/.flow/docs` to the clone's `docs/`, so a skill names the manual by a fixed path rather than `~/.flow/scripts/../docs/`. And on a machine where `~/.flow/snapshots/` is empty it writes the manifest of what it is about to create before creating it, so restore reaches the bootstrap links too. `management.md` → `## What a machine installs`, `### Setup opens in a terminal, not a session`, `### A stale name in ~/.local/bin`
...
- [ ] **Working on more than one machine.** Raised by the user 2026-09-09 as `~/.flow/` getting a git repository of its own, and widened 2026-09-19. The user often leaves work half done on one machine and carries on at another. The design gets its own record, `lab/context/multi-machine.md`, written when the design starts. **talk first**. The parts:
  - **`~/.flow/` becomes a git repository with a private remote.** Travels: `settings.json`, `workflow-notes.md`, `study-cases/`, `private-skills/`, and the work made outside any project, `tickets/` and `groundwork/`. Stays on the machine: `snapshots/` and `migrations/`, which name this machine's paths; `audit/audit.db`, derived and 44 MB; `scorecards/`, per session; `scripts/` and `references/`, links into the clone.
  - **Sync runs far more often than a session's end, both ways, with no command.** `SessionEnd` fires only on exit, `/clear`, `/resume` and `/logout`, never on `/compact`. Its hooks share a 1.5-second budget, too short for a push, and a session left open on one machine never ends. Proposed 2026-09-19: a background commit and push from `Stop` after every answer that changed `~/.flow/`, and a background fast-forward pull from `SessionStart` and from `UserPromptSubmit` once the last pull is 5 minutes old. A pull that cannot fast-forward stops and says so.
  - **A project travels through its own repository.** Its `.flow/`, tickets included, is committed with it, so only uncommitted work stays behind. `util git save` stages, commits and pushes all of it in one command. Open: whether the project side runs automatically too, since a commit after every answer fills the project's history with half-done work.
  - **The user profile sits outside `~/.flow/`**: `## The user` and `## Preferences` in `~/.agents/AGENTS.md`. **One state path across harnesses**, below, covers moving it.
