...
- [ ] **Re-ask the 6 questions from `shit-explanations.md`** in a scratch session through `bash scripts/try.sh`, against the rebuilt `## Explaining`. The rewrite is unmeasured until then, and 5 earlier rounds of rule-writing were never tested either
...
- [x] **`## Explaining` rebuilt — done 2026-08-29.** *Resolve the referent* landed inside *Answer a many-topic message topic by topic*. The section is now grouped message → sentence → word, 3 contradictions are gone, and *Name the subject first* leads it — the fault behind 5 of the 6 rejected messages. `shit-explanations.md`
...
- [x] **Read the two ADHD skills — done 2026-08-29.** `repos/adhd` is a divergent-ideation engine and gave nothing. `repos/i-have-adhd` gave 1 rule of 10: say what now works, in concrete terms. Both optimise for brevity, and none of the 6 rejected messages failed on length — 3 of the 6 root causes name compression as the cause
