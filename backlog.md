...
8. **The writing pass over `references/`, `docs/`, `home/settings.md` and the 2 script templates**
9. **Widen the tests** over `flow install` and the ticket commands
...
- [ ] **`flow doctor`: everything about an installed machine a function can decide.** Every symlink resolves, `flow` and `util` on `PATH` point at this clone rather than the repository deleted 2026-08-07, `~/.claude/` and `~/.flow/` hold the files they should, both hooks are registered in `settings.json`, both suites pass. It goes in code because a skill checking 20 symlinks by hand gets it wrong and reports success. The management skill runs it and then walks what no function can reach. Named `doctor` rather than `check` by the user 2026-09-08, matching Claude Code's own `/doctor`; `flow check` has never existed, whatever `state.md` said. **v1**
...
- [ ] **A rule saying the user is always short on time, so a long answer costs them.** Ruled by the user 2026-09-10 after a reply that walked one command through a fixture, its output, two failure cases and a Flow tie-in, when the question was only what the output looks like. Their words: "do not fucking expect me to fully read it", "user is always short on time", "your last response is absolute blot". `## Explaining` already carries `size-by-worth`, which sizes a section by what the topic is worth to the reader, and it did not fire: it reads as a licence to be long whenever the topic is worth it. The missing half is that the reader's time is the budget, so **short is the default and length is spent, never earned**. Belongs in `home/CLAUDE.md` under `## Explaining` and in the repo `CLAUDE.md`, worded once. **v1**
...
- [ ] **The `util` repository was never swept for em dashes.** `references/style.md` bans them and records a sweep of this repo on 2026-09-08; the submodule was not in it. 20 sites across `util.js`, `lib/`, `builtin/`, `commands/git/save.sh` and `tests/util.test.js`, and 6 of them reach the user: the `util ls` title, a namespace heading, and 4 refusal messages. `tests/util.test.js:158` asserts on one, so the sweep edits a test as well as the strings
...
- [ ] **`docs/dev/` and `docs/manual/` may be one folder.** `dev/` serves whoever works on Flow and `manual/` whoever uses it, and for now the author is both, so every new page costs a routing decision before a word gets written. The user raised merging them so the documentation push has one place to land. Against: `manual/` is a v1 deliverable with a designed 6-section shape in `design-public-docs.md`, and `dev/` is working notes for one reader. **talk first**
...
- [ ] **`docs/manual/`: write Reference, then Use Flow.** The design is 6 sections for a stranger who cloned the repo, and the first reader is the author, who forgets which features exist. Reference is every command, skill, setting and file in one place, and it is the page that cures that. Use Flow is what Flow is, the 4 phases, tickets and the approval discipline. The other 4 wait: `Why it works this way` needs `lab/context/` read end to end, about 4,000 lines, and `Configure`, `Extend` and `Work on Flow` overlap `docs/dev/`. Moved into v1 by the user 2026-09-08. `tickets.md` was written first, 2026-09-09, on the user's instruction: it is the ticket shape end to end plus one filled ticket, and Reference has to point at it rather than restate it. **Reference is the features list**, one entry per unit, and it is where a mechanism used in more than one place gets defined. The `open` block set that precedent by leaving `tickets.md` for `util`: a page holds a reader's question, and a feature too small for one belongs in Reference. **v1**. `design-public-docs.md`
