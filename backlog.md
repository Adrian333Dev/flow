...
- [ ] **What a changelog entry holds, so a migration can act on it.** `CHANGELOG.md` came back on 2026-09-20 with entry `1`, which unblocks every line below this one. `CLAUDE.md` → `no-changelog-entry-yet` holds the writing rule, and the second entry waits for the first machine. The mechanism around it: `~/.flow/version` is one line holding the number of the newest entry a machine applied, step 2 of a migration reads every entry above that number, `flow doctor` compares the 2, and the git tag a machine installs from is named for the entry, `v12`. What is left to settle: what an entry says so the agent can write a migration from it, beyond today's rule of one line per change of behavior; and whether an entry may say "the old skill cannot carry this out", which `## When a migration changes the management skill itself` leans on. **talk first** `management.md` → `## The version`, `## The changelog comes back`

- [ ] **`flow doctor` gains the version checks.** `~/.flow/run.json` on disk is reported before anything else as a problem, exit non-zero, naming the step the run stopped at and the 2 ways out. Three comparisons are notes with `flow up` suggested and exit 0: the machine's `~/.flow/version` against the number of the newest changelog entry, a project's `.flow/version` against the machine's, and the clone against the newest tag, that last one behind `flow doctor --updates` because it is the only check that touches the network. A `~/.flow/version` above the newest entry's number is a problem, since the clone moved backwards. A submodule sitting off its gitlink is a note, never a problem. `management.md` → `## The version`, `## Prerequisites are checked by running them`
