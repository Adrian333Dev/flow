...
- [ ] **A dropped file path costs a whole extra turn**: dragging a file from the editor into the terminal pastes its absolute path in quotes, and that is the only easy way to name a file `@` cannot find or that git ignores. The agent then spends one turn seeing the path and a second reading the file. Wanted: the content arrives with the prompt. **Researched 2026-09-21, 2 mechanisms found and neither designed.** `fileSuggestion` in `settings.json` replaces the `@` menu with a script of ours: it is handed `{"query": "src/comp"}` on stdin and answers with up to 15 paths, which is how a git-ignored file gets into the menu. A `UserPromptSubmit` hook covers the drop itself: it cannot rewrite the prompt, only add context beside it, so it reads the dropped path and prints the file, capped at 10,000 characters and 30 seconds. **Proved by a scratch session the same day: `@` expands a git-ignored path and delivers the content, with Read, Bash, Grep and Glob all disallowed**, so only the suggestion menu filters by git, never the expansion. **talk first**. `lab/research/claude-code-docs/hooks.md`, `settings.md` → `### File suggestion settings`
...
- [ ] **Flow on another harness and on another model.** Researched 2026-09-06 and 2026-09-07, and nothing is locked. **Codex waits until Flow has shipped on Claude Code**, ruled by the user 2026-09-18, reversing a ruling from the same morning that put the port before the management skill. Each harness may get its own mechanisms rather than one bent to fit both. The `flow:` prefix and the one real copy of each file in `~/.agents/` were built before the reversal and stay, since any harness needs them. Building it for a stranger split off into `## V1` on 2026-09-16. **talk first**. `models.md`. The parts:
  ...
  - **A skill linked onto the whole machine never reaches Codex.** `flow private-skills add --global` links into `~/.claude/skills/`, and `flow domain-skills add --global`, built 2026-09-20, does the same. Codex reads `~/.agents/skills/` alone, so both would move there, with a link back for Claude Code the way Flow's own plugin folder has one
